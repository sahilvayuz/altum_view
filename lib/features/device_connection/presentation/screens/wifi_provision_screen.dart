...
class _WifiProvisionScreenState extends State<WifiProvisionScreen> {
  ...
  void dispose() {
    ...
    context.read<WifiController>().disposeController();
    _ssidCtrl.dispose();
    _passCtrl.dispose();
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    return Consumer<DeviceConnectionController>(
      builder: (context, ctrl, _) {
        // ── Success ───────────────────────────────────────────────────────────
        if (ctrl.step == SetupStep.success) {
        ...
        // ── Error ─────────────────────────────────────────────────────────────
        if (ctrl.step == SetupStep.error && _submitted) {
          return _ErrorScreen(
        ...
        // ── Progress ──────────────────────────────────────────────────────────
        if (ctrl.step == SetupStep.progress) {
          return _ProgressScreen(message: ctrl.statusMessage);
        }
                ...
                children: [
                  ...
                  if (ctrl.wifiList.isNotEmpty) ...[
                    ...
                    SizedBox(
                      ...
                      child: ListView.separated(
                        ...
                        itemCount:        ctrl.wifiList.length,
                        separatorBuilder: (_, __) => const SizedBox(width: 8),
                        itemBuilder: (_, i) {
                          final ssid = ctrl.wifiList[i];
                          return GestureDetector(
                            onTap: () => _ssidCtrl.text = ssid,
                            child: Container(
                  ...
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      prefixIcon: Icon(CupertinoIcons.wifi, color: AppTheme.primary),
                    ),
                  ),
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      onPressed: () async {
                        ...
                        await ctrl.submitWifi(ssid, pass);
                      },
